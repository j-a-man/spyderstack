      ...
      <div className="container mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-1 gap-12">
          {caseStudies.map((study, index) => (
            <div 
              key={index} 
              className="group relative bg-white/5 border border-white/10 overflow-hidden hover:border-primary/50 transition-all duration-500"
            >
              {/* Subtle Hover Glow */}
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 p-8 md:p-12">
                ...
                <div className="lg:col-span-5 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <span className="text-muted-foreground text-sm tracking-wide border-l border-white/10 pl-4">{study.location}</span>
                    </div>
                    <h3 className="font-display text-4xl font-bold text-white mb-2">{study.company}</h3>
                  </div>

                  {/* Metric Grid */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-8 lg:mt-0 py-8 border-t border-b border-white/10">
                    <div>
                      <div className="text-3xl font-bold text-white mb-1 group-hover:text-primary transition-colors font-display">{study.results.missedCallsReduced}</div>
                      <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Calls Saved</div>
                    </div>
                    <div>
                      <div className="text-3xl font-bold text-white mb-1 group-hover:text-primary transition-colors font-display">{study.results.responseTime}</div>
                      <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Response</div>
                    </div>
                    <div>
                      <div className="text-3xl font-bold text-white mb-1 group-hover:text-primary transition-colors font-display">{study.results.revenueIncrease}</div>
                ...
                <div className="lg:col-span-7 flex flex-col justify-center pl-0 lg:pl-12 border-l-0 lg:border-l border-white/10">
                  <blockquote className="text-xl md:text-2xl text-muted-foreground leading-relaxed italic mb-8">
                    "{study.quote}"
                  </blockquote>
                  <div className="flex items-center gap-4">
                    <div className="h-12 w-12 rounded-full bg-white/10 flex items-center justify-center text-white font-bold border border-white/5">
                      {study.name[0]}
                    </div>
                    <div>
...
function PortfolioCTA() {
  return (
    <section className="relative z-10 py-32 bg-primary border-t border-white/20 overflow-hidden text-center">
