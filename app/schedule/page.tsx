...
export default function SchedulePage() {
        ...
        try {
            const res = await fetch('/api/book', {
                ...
                body: JSON.stringify({
                    ...
                    // CHANGED: Sending EST explicitly to the backend for email clarity
                    time: selectedTime ? `${selectedTime} EST` : null
                })
            })
    ...
    const isSlotFull = (time: string) => {
        ...
        // Check using the raw time string to match DB keys
        const key = `${selectedDate.toLocaleDateString()}-${time} EST`
                ...
                <section className="relative z-10 pt-32 pb-20 px-6 min-h-screen flex flex-col justify-center">
                    <div className="container mx-auto max-w-5xl">
                        ...
                        <div className="text-center mb-16">
                            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-muted/50 border border-border backdrop-blur-md mb-6 animate-in fade-in zoom-in duration-700">
                        ...
                        {/* Main Card Panel */}
                        <div className="bg-background border border-border shadow-2xl relative overflow-hidden min-h-[600px] flex flex-col md:flex-row">
                            {/* Top Progress Bar (Mobile) */}
                            <div className="absolute top-0 left-0 right-0 h-1 bg-muted/50 md:hidden">
                                <div className="h-full bg-primary transition-all duration-500" style={{ width: `${(step / 3) * 100}%` }} />
                            </div>

                            {/* LEFT SIDE: Summary */}
                            <div className="md:w-1/3 bg-muted/50 border-r border-border p-8 md:p-12 flex flex-col justify-between">
                                <div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-foreground font-medium">
                                                    {/* CHANGED: Added "EST" here for clarity */}
                                                    {selectedTime ? `${selectedTime} EST` : "Select a Time"}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {step === 1 && (
                                    <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                                        <div className="flex justify-between items-end mb-6">
                                            <h3 className="font-display text-2xl font-bold text-foreground">SELECT DATE & TIME</h3>
                                            <span className="text-xs text-primary uppercase tracking-wider font-bold">EST Timezone</span>
                                        </div>
                                                ...
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        selectedDate?.toDateString() === date.toDateString()
                                                            ? "bg-primary border-primary text-primary-foreground shadow-[0_0_20px_-5px_var(--primary)]"
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={cn(
                                                                ...
                                                                (!selectedDate || full) && "opacity-30 cursor-not-allowed border-transparent bg-muted/50"
                                                            )}
                                                        >
                                                            {time}
                                        ...
                                        <div className="mt-8 flex justify-end">
                                            <Button
                                                ...
                                                onClick={() => setStep(2)}
                                                className="h-12 px-8 rounded-none bg-primary hover:bg-primary/90 text-primary-foreground uppercase tracking-widest font-bold disabled:opacity-50 disabled:cursor-not-allowed"
                                            >
                                        ...
                                        <form onSubmit={handleBooking} className="space-y-6">
                                            ...
                                            <div className="pt-4 flex gap-4">
                                                <Button
                                                    ...
                                                    variant="outline"
                                                    onClick={() => setStep(1)}
                                                    className="h-12 px-8 rounded-none border-border text-foreground hover:bg-muted/50 uppercase tracking-widest font-bold"
                                                >
                                                    Back
                                                </Button>
                                                <Button
                                                    type="submit"
                                                    disabled={loading}
                                                    className="h-12 px-8 flex-1 rounded-none bg-primary hover:bg-primary/90 text-primary-foreground uppercase tracking-widest font-bold"
                                                >
                                ...
                                {step === 3 && (
                                    <div className="h-full flex flex-col justify-center items-center text-center animate-in fade-in zoom-in duration-500">
                                        ...
                                        <Button
                                            ...
                                            className="h-12 px-8 rounded-none bg-foreground text-background hover:bg-foreground/90 uppercase tracking-widest font-bold"
                                        >
                                            Return to Base
                                        </Button>
                                    </div>
